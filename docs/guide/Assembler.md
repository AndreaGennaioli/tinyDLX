...
```bash
python tools/asm.py input.asm output.bin
```
...
For now, a line that defines a label (like `LABEL:`, see below) cannot also contain an instruction.
...
An immediate can be written in decimal, including negative values, in hexadecimal with `0x` or in binary with `0b`.

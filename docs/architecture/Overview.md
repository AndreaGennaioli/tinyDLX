...
- **Special registers**: SR holds the interrupt enable flag IEN, IAR the address at which execution resumes after an interrupt, CR the cause of the last interrupt. No instruction reads or writes them directly: they change only on interrupt entry and on `RFE`, as described in [Interrupts.md](./Interrupts.md).
- **Memory**: byte-addressed and big-endian, see [Memory.md](./Memory.md).

## How this section is organised

- [ISA.md](./ISA.md): instruction formats and the full instruction set.
- [Memory.md](./Memory.md): program loading, endianness, alignment and the address map.
...
- **`INT` instead of `TRAP`**: hardware and software interrupts share the entry point at address 0, and the Cause Register tells them apart.
